...
data class InvoicePerson(
    ...
    companion object {
        fun of(person: PersonDetailed, restrictedAddress: InvoiceAddress): InvoicePerson {
            val name = person.invoiceRecipientName.trim()
                .ifEmpty { "${person.lastName.trim()} ${person.firstName.trim()}".trim() }
            val address = if (person.restrictedDetailsEnabled) restrictedAddress else address(person)
        ...
        private fun address(person: PersonDetailed): InvoiceAddress {
            ...
            return if (invoicingStreetAddress.isNotEmpty() && invoicingPostalCode.isNotEmpty() && invoicingPostOffice.isNotEmpty()) {
                InvoiceAddress(
