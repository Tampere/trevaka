...
import fi.nokiankaupunki.evaka.invoice.config.findProduct
import trevaka.invoice.FixedLengthField
import trevaka.invoice.FixedLengthField.Cents
import trevaka.invoice.FixedLengthField.Date
import trevaka.invoice.FixedLengthField.Empty
import trevaka.invoice.FixedLengthField.Number
import trevaka.invoice.FixedLengthField.Text
import trevaka.invoice.InvoiceAddress
import trevaka.invoice.InvoicePerson
...
private val RESTRICTED_ADDRESS = InvoiceAddress(
    "Sivistyspalvelut, Varhaiskasvatuksen asiakaslaskustus, Harjukatu 23",
    "37100",
    "Nokia",
)

class NokiaInvoiceClient(
    ...
    override fun send(invoices: List<InvoiceDetailed>): SendResult {
        ...
        val (nonSsnInvoices, ssnInvoices) = nonZeroSumInvoices.partition { invoice -> invoice.headOfFamily.ssn == null }
...
internal fun toInvoiceRows(header: String, rows: List<InvoiceRowDetailed>): List<List<FixedLengthField>> = rows.flatMap { row ->
    listOfNotNull(
        toDescriptionRow(header, "${row.periodStart.format(DATE_FORMATTER)} - ${row.periodEnd.format(DATE_FORMATTER)}"),
...
internal fun toInvoiceHeaderRow(header: String, invoice: InvoiceDetailed, now: HelsinkiDateTime, properties: InvoiceProperties): List<FixedLengthField> {
    ...
    return listOf(
        ...
        Text(if (person.name.length > 50) person.name.substring(50, min(100, person.name.length)) else null, 50), // 5. Asiakkaan nimi 2
        ...
        Text(null, 11), // 37. Hetu / y-tunnus: Asiakkaan hetu / y-tunnus (virallinen) mikäli ei tuoda kohdassa 1
        ...
        Text(null, 70), // 151. TenderReference: Vain yritysasiakkaat: TenderReference (Kilpailutuksen numero)
        Text(null, 35), // 152. Tiliöintiehdotus: Vain yritysasiakkaat: Tiliöintiehdotus ostajan kirjanpitoa varten
 ...
 */
internal fun toCodebtorRow(header: String, codebtor: PersonDetailed): List<FixedLengthField> {
    ...
    return listOf(
        ...
        Text("${person.address.postalCode} ${person.address.postOffice}", 30), // 134. Postiosoite: Yhteisvelallisen postinumero ja postitoimipaikka
 ...
 */
internal fun toInvoiceRow(header: String, row: InvoiceRowDetailed): List<FixedLengthField> = listOf(
    ...
    Cents(abs(row.unitPrice), 8, 4), // 81. Yksikköhinta: Yksikköhinta (Sovittaessa käyttöön useampi desimaali, esim. Vesikanta)
    Text("KPL", 3), // 82. Yksikkö: Yksikkö (isoilla kirjaimilla), esim. KPL, KG, H, jne.
    Text("-".takeIf { row.unitPrice < 0 }, 1), // 83. Lkm-etumerkki: Laskutettavien yksiköiden lukumäärän etumerkki
    ...
    Text(
        when (row.unitProviderType) {
            ProviderType.MUNICIPAL -> "5073"
            ProviderType.PURCHASED -> "5024"
            else -> throw Error("ProviderType ${row.unitProviderType} not supported")
        },
        4,
    ), // TOIM: Toimintokoodi
    ...
    Text(
        when {
            row.daycareType.contains(CareType.FAMILY) -> "1231"
            row.daycareType.contains(CareType.GROUP_FAMILY) -> "1231"
            row.daycareType.contains(CareType.CENTRE) -> "1190"
    ...
    Text("-".takeIf { row.price < 0 }, 1), // 90A. Summa-etumerkki: Laskurivin rivisumman etumerkki
    Cents(abs(row.price), 9, 2), // 90B. Rivisumma: Laskurivin rivisumma (voi myös sisältää etumerkin pos. 265)
)

/**
 * 3.7 Tekstimuotoisen laskurivin kuvaus
 */
internal fun toDescriptionRow(header: String, description: String): List<FixedLengthField> = listOf(
