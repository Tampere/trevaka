...
package fi.hameenkyro.evaka.invoice.config
...
import fi.hameenkyro.evaka.invoice.service.ProEInvoiceGenerator
import fi.hameenkyro.evaka.invoice.service.S3Sender
...
class InvoiceConfiguration {
    @Primary
    @Bean(name = ["hameenkyroInvoiceIntegrationClient"])
    fun invoiceIntegrationClient(
        ...
        return HameenkyroInvoiceIntegrationClient(clockService, s3Sender, invoiceGenerator)
    }

    @Bean
    fun incomeTypesProvider(): IncomeTypesProvider = HameenkyroIncomeTypesProvider()

    @Bean
    fun incomeCoefficientMultiplierProvider(): IncomeCoefficientMultiplierProvider = HameenkyroIncomeCoefficientMultiplierProvider()

    @Bean
    fun invoiceProductProvider(): InvoiceProductProvider = HameenkyroInvoiceProductProvider()
...
class HameenkyroInvoiceProductProvider : InvoiceProductProvider {
