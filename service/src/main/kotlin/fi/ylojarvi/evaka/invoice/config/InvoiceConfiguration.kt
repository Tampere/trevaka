...
package fi.ylojarvi.evaka.invoice.config
...
class InvoiceConfiguration {
    @Primary
    @Bean(name = ["ylojarviInvoiceIntegrationClient"])
    fun invoiceIntegrationClient(
        clockService: ClockService,
        properties: YlojarviProperties,
    ): InvoiceIntegrationClient {
        val sftpEnv = properties.invoice.sftp.toSftpEnv()
        return YlojarviInvoiceClient(SftpClient(sftpEnv), properties.invoice, clockService)
    }

    @Bean
    fun incomeTypesProvider(): IncomeTypesProvider = YlojarviIncomeTypesProvider()

    @Bean
    fun incomeCoefficientMultiplierProvider(): IncomeCoefficientMultiplierProvider = YlojarviIncomeCoefficientMultiplierProvider()

    @Bean
    fun invoiceProductProvider(): InvoiceProductProvider = YlojarviInvoiceProductProvider()
...
class YlojarviInvoiceProductProvider : InvoiceProductProvider {
