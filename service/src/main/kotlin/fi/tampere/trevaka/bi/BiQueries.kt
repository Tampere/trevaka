...
object BiQueries {
    ...
    val getAssistanceFactors =
        csvQuery<BiAssistanceFactor> {
            sql(
                """
            select id, created, updated, child_id, modified, modified_by, valid_during, capacity_factor
    ...
    val getAssistanceNeedVoucherCoefficients =
        csvQuery<BiAssistanceNeedVoucherCoefficient> {
            sql(
                """
            select id, created, updated, child_id, validity_period, coefficient
    ...
    val getCurriculumTemplates =
        csvQuery<BiCurriculumTemplate> {
            sql(
                """
            select id, created, updated, valid, type, language, name, content::text
    ...
    val getDaycareAssistances =
        csvQuery<BiDaycareAssistance> {
            sql(
                """
            select id, created, updated, child_id, modified, modified_by, valid_during, level
    ...
    val getFeeDecisions =
        csvQuery<BiFeeDecision> {
            sql(
                """
            select id, created, updated, status, valid_during, decision_type, head_of_family_id, head_of_family_income::text, partner_id, partner_income::text, family_size, fee_thresholds::text, decision_number, document_key, approved_at, approved_by_id, decision_handler_id, sent_at, cancelled_at, total_fee, difference
            FROM fee_decision
        """
            )
        }
    ...
    val getHolidayPeriods =
        csvQuery<BiHolidayPeriod> {
            sql(
                """
            select id, created, updated, period, reservation_deadline
            FROM holiday_period
        """
            )
        }
    ...
    val getHolidayQuestionnaireAnswers =
        csvQuery<BiHolidayQuestionnaireAnswer> {
            sql(
                """
            select id, created, updated, modified_by, questionnaire_id, child_id, fixed_period
    ...
    val getOtherAssistanceMeasures =
        csvQuery<BiOtherAssistanceMeasure> {
            sql(
                """
            select id, created, updated, child_id, modified, modified_by, valid_during, type
    ...
    val getServiceNeedOptionVoucherValues =
        csvQuery<BiServiceNeedOptionVoucherValue> {
            sql(
                """
            select id, created, updated, service_need_option_id, validity, base_value, coefficient, value, base_value_under_3y, coefficient_under_3y, value_under_3y
    ...
    private fun printTampereBiCsvField(value: Any?): String = printCsvField(value).replace("\"", "")
    ...
    class StreamingCsvQuery<T : Any>(
        ...
        override operator fun <R> invoke(
            ...
            query(tx).useSequence { rows ->
                useResults(toCsvRecords(::printTampereBiCsvField, clazz, rows))
            }
    }
