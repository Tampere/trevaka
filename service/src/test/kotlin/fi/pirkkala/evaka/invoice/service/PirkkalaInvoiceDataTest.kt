...
class PirkkalaInvoiceDataTest {
    ...
    @Test
    fun `descriptionRowFields is in order`() {
        assertInvoiceFieldsAreInOrder(descriptionRowFields)
    }
}
...
private fun assertInvoiceFieldsAreInOrder(fields: List<InvoiceField>) {
    for (i in 1..<fields.size) {
        ...
        assertEquals(currentStartPos, previousEndPos) { "Check ${previous.field} & ${current.field} start/length fields" }
    }
}
