...
class PirkkalaInvoiceNumberTest : AbstractPirkkalaIntegrationTest() {
    ...
    fun `first invoice has correct number`() {
        ...
        assertThat(invoices).extracting<Long?> { it.number }.containsExactly(53000001)
    }
    ...
    fun `second invoice has correct number`() {
        val invoices = db.transaction { tx ->
            ...
            tx.insert(
                DevInvoice(
                    ...
                    number = 53000001, // current series start
        ...
        assertThat(invoices).extracting<Long?> { it.number }.containsExactly(53000002)
    }
}
