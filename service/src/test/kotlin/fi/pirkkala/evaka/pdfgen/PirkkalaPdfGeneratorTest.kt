...
package fi.pirkkala.evaka.pdfgen
...
class PirkkalaPdfGeneratorTest : AbstractPirkkalaIntegrationTest() {
    ...
    fun createDecisionPdf(decisionType: DecisionType, providerType: ProviderType, isTransferApplication: Boolean) {
        ...
        val filename = "pirkkala-decision-$decisionType-$providerType${if (isTransferApplication) "-transfer" else ""}.pdf"
    ...
    fun generateFeeDecisionPdf(placementType: PlacementType) {
        ...
        val filename = "pirkkala-fee-decision-with-placement-type-$placementType.pdf"
        writeReportsFile(filename, bytes)
    }
    ...
    fun generateFeeDecisionPdf(decisionType: FeeDecisionType) {
        ...
        val filename = "pirkkala-fee-decision-with-decision-type-$decisionType.pdf"
        writeReportsFile(filename, bytes)
    }
    ...
    fun generateVoucherValueDecisionPdf(decisionType: VoucherValueDecisionType) {
        ...
        val filename = "pirkkala-voucher-value-decision-with-decision-type-$decisionType.pdf"
        writeReportsFile(filename, bytes)
    }
    ...
    fun daycareAssistanceDecisionGeneratePdf() {
        ...
        val filename = "pirkkala-daycare-assistance-decision.pdf"
        writeReportsFile(filename, bytes)
    }
    ...
    fun preschoolAssistanceDecisionGeneratePdf() {
        ...
        val filename = "pirkkala-preschool-assistance-decision.pdf"
        writeReportsFile(filename, bytes)
    }
}
