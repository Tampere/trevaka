...
package fi.kangasala.evaka.export
...
private fun hasExtendedCompulsoryEducation(level: PreschoolAssistanceLevel) = when (level) {
...
class PreschoolerExportTest : AbstractKangasalaIntegrationTest() {
    ...
    fun `mimimal data`() {
        ...
        assertThat(row.`esioppilaan postinumero`).isEqualTo("${child.postalCode} ${child.postOffice}")
        ...
        assertThat(row.`huoltajan 1 postinumero`).isEqualTo("${guardian1.postalCode} ${guardian1.postOffice}")
        ...
        assertThat(row.`huoltajan 2 postinumero`).isEqualTo("${guardian2.postalCode} ${guardian2.postOffice}")
