...
package fi.orivesi.evaka.pdfgen
...
class OrivesiPdfGeneratorTest : AbstractOrivesiIntegrationTest() {
    ...
    fun createDecisionPdf(decisionType: DecisionType, providerType: ProviderType, isTransferApplication: Boolean) {
        ...
        val filename = "orivesi-decision-$decisionType-$providerType${if (isTransferApplication) "-transfer" else ""}.pdf"
    ...
    fun generateFeeDecisionPdf(placementType: PlacementType) {
        ...
        val filename = "orivesi-fee-decision-with-placement-type-$placementType.pdf"
        writeReportsFile(filename, bytes)
    }
    ...
    fun generateFeeDecisionPdf(decisionType: FeeDecisionType) {
        ...
        val filename = "orivesi-fee-decision-with-decision-type-$decisionType.pdf"
        writeReportsFile(filename, bytes)
    }
    ...
    fun generateVoucherValueDecisionPdf(decisionType: VoucherValueDecisionType) {
        ...
        val filename = "orivesi-voucher-value-decision-with-decision-type-$decisionType.pdf"
        writeReportsFile(filename, bytes)
    }
    ...
    fun daycareAssistanceDecisionGeneratePdf() {
        ...
        val filename = "orivesi-daycare-assistance-decision.pdf"
        writeReportsFile(filename, bytes)
    }
    ...
    fun preschoolAssistanceDecisionGeneratePdf() {
        ...
        val filename = "orivesi-preschool-assistance-decision.pdf"
        writeReportsFile(filename, bytes)
    }
}
