...
private fun extendedCompulsoryEducationFn(level: PreschoolAssistanceLevel) = when (level) {
...
class PreschoolerExportTest : AbstractTampereIntegrationTest() {
    ...
    fun `mimimal data`() {
        db.transaction { tx ->
            ...
            val childId = tx.insert(DevPerson(dateOfBirth = LocalDate.of(2019, 1, 1)), DevPersonType.CHILD)
            tx.insert(
                DevPlacement(
        ...
        val rows = db.read { tx ->
            tx.createQuery { sql("SELECT * FROM preschooler_export(2019, ${bind(date)})") }.toList<PreschoolerExportRow>()
        }
    ...
    fun `full data`() {
        ...
        val expected = PreschoolerExportRow(
            "010119A9555",
            "Ankka",
            "Aku",
            "Ankkalinnake",
        ...
        db.transaction { tx ->
            val unitId = tx.insert(DevDaycare(areaId = areaId, name = expected.`esioppilaan esiopetuspaikka`))
            val groupId = tx.insert(DevDaycareGroup(daycareId = unitId, name = expected.`esioppilaan esiopetusryhmä`!!))
            ...
            tx.insert(DevDaycareGroupPlacement(daycarePlacementId = placementId, daycareGroupId = groupId, startDate = date, endDate = date))
            tx.insert(DevPreschoolAssistance(childId = childId, validDuring = FiniteDateRange(date, date), level = PreschoolAssistanceLevel.INTENSIFIED_SUPPORT))
        }
    ...
    fun `wrong year of birth`() {
        db.transaction { tx ->
            val unitId = tx.insert(DevDaycare(areaId = areaId))
            val child1 = tx.insert(DevPerson(dateOfBirth = LocalDate.of(2017, 12, 31)), DevPersonType.CHILD)
            tx.insert(
                DevPlacement(
                    type = PlacementType.PRESCHOOL,
                    childId = child1,
            ...
            val child2 = tx.insert(DevPerson(dateOfBirth = LocalDate.of(2020, 1, 1)), DevPersonType.CHILD)
            tx.insert(
                DevPlacement(
                    type = PlacementType.PRESCHOOL,
                    childId = child2,
    ...
    fun `6-year-old with extended compulsory education is not included`(level: PreschoolAssistanceLevel) {
        db.transaction { tx ->
            val unitId = tx.insert(DevDaycare(areaId = areaId))
            val childId = tx.insert(DevPerson(dateOfBirth = LocalDate.of(2019, 1, 1)), DevPersonType.CHILD)
            tx.insert(
        ...
        if (extendedCompulsoryEducation) {
            ...
            assertThat(rows).extracting<String?> { it.`esioppilaan voimassa oleva tuen taso` }
                .hasSize(1)
                .first()
                .isNotNull
                .isNotEqualTo("")
    ...
    fun `7-year-old is always included`(level: PreschoolAssistanceLevel) {
        db.transaction { tx ->
            val unitId = tx.insert(DevDaycare(areaId = areaId))
            val childId = tx.insert(DevPerson(dateOfBirth = LocalDate.of(2018, 12, 31)), DevPersonType.CHILD)
            tx.insert(
                DevPlacement(
                    type = PlacementType.PRESCHOOL,
                    childId = childId,
