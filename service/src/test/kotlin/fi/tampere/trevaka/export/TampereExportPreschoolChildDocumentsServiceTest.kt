...
import fi.espoo.evaka.document.Question.CheckboxGroupQuestion
import fi.espoo.evaka.document.Question.CheckboxQuestion
import fi.espoo.evaka.document.Question.DateQuestion
import fi.espoo.evaka.document.Question.GroupedTextFieldsQuestion
import fi.espoo.evaka.document.Question.RadioButtonGroupQuestion
import fi.espoo.evaka.document.Question.StaticTextDisplayQuestion
...
class TampereExportPreschoolChildDocumentsServiceTest : AbstractTampereIntegrationTest() {
    ...
    fun `question type works`(questionType: QuestionType) {
        ...
        val answer = when (questionType) {
            ...
            QuestionType.CHECKBOX_GROUP -> AnsweredQuestion.CheckboxGroupAnswer(
                questionId,
                answer = listOf(CheckboxGroupAnswerContent(optionId = "1"), CheckboxGroupAnswerContent(optionId = "2")),
            )

            QuestionType.RADIO_BUTTON_GROUP -> AnsweredQuestion.RadioButtonGroupAnswer(questionId, answer = "1")

            QuestionType.STATIC_TEXT_DISPLAY -> AnsweredQuestion.StaticTextDisplayAnswer(questionId, answer = null)

            QuestionType.DATE -> AnsweredQuestion.DateAnswer(questionId, answer = LocalDate.of(2025, 1, 2))

            QuestionType.GROUPED_TEXT_FIELDS -> AnsweredQuestion.GroupedTextFieldsAnswer(
                questionId,
                answer = listOf(
                    listOf("vastaus rivi 1 sarake 1", "vastaus rivi 1 sarake 2", "vastaus rivi 1 sarake 3"),
                    listOf("vastaus rivi 2 sarake 1", "vastaus rivi 2 sarake 2", "vastaus rivi 2 sarake 3"),
                ),
            )
        }
...
private fun AnsweredQuestion<*>.toQuestion(label: String) = when (this) {
    is AnsweredQuestion.CheckboxAnswer -> CheckboxQuestion(questionId, label)

    is AnsweredQuestion.CheckboxGroupAnswer -> CheckboxGroupQuestion(
        questionId,
        label,
        options = answer.map {
    ...
    is AnsweredQuestion.GroupedTextFieldsAnswer -> GroupedTextFieldsQuestion(
        questionId,
        label,
        fieldLabels = if (answer.isNotEmpty()) {
            answer.first()
                .mapIndexed { index, _ -> "sarake ${index + 1}" }
        } else {
            emptyList()
        },
    ...
    is AnsweredQuestion.RadioButtonGroupAnswer -> RadioButtonGroupQuestion(
        questionId,
        label,
...
private fun AnsweredQuestion<*>.toExpectedJson(): String = when (this) {
    is AnsweredQuestion.CheckboxAnswer -> answer.toString()

    is AnsweredQuestion.CheckboxGroupAnswer -> "[${answer.joinToString(", ") { "\"vastaus ${it.optionId}\"" }}]"

    is AnsweredQuestion.DateAnswer -> answer?.let { "\"${it.format(DateTimeFormatter.ofPattern("dd.MM.yyyy"))}\"" }
        ?: "null"
