...
import fi.espoo.evaka.shared.security.Action.Application
...
import fi.espoo.evaka.shared.security.Action.BackupPickup
import fi.espoo.evaka.shared.security.Action.CalendarEvent
import fi.espoo.evaka.shared.security.Action.CalendarEventTime
import fi.espoo.evaka.shared.security.Action.Child
import fi.espoo.evaka.shared.security.Action.ChildDailyNote
import fi.espoo.evaka.shared.security.Action.ChildDocument
import fi.espoo.evaka.shared.security.Action.ChildImage
import fi.espoo.evaka.shared.security.Action.ChildStickyNote
import fi.espoo.evaka.shared.security.Action.ClubTerm
import fi.espoo.evaka.shared.security.Action.DailyServiceTime
import fi.espoo.evaka.shared.security.Action.DaycareAssistance
import fi.espoo.evaka.shared.security.Action.Decision
import fi.espoo.evaka.shared.security.Action.DocumentTemplate
import fi.espoo.evaka.shared.security.Action.Employee
import fi.espoo.evaka.shared.security.Action.FeeAlteration
import fi.espoo.evaka.shared.security.Action.FeeDecision
import fi.espoo.evaka.shared.security.Action.FeeThresholds
import fi.espoo.evaka.shared.security.Action.FinanceNote
import fi.espoo.evaka.shared.security.Action.FosterParent
import fi.espoo.evaka.shared.security.Action.Global
import fi.espoo.evaka.shared.security.Action.Group
import fi.espoo.evaka.shared.security.Action.GroupNote
import fi.espoo.evaka.shared.security.Action.GroupPlacement
import fi.espoo.evaka.shared.security.Action.Income
import fi.espoo.evaka.shared.security.Action.IncomeStatement
import fi.espoo.evaka.shared.security.Action.Invoice
import fi.espoo.evaka.shared.security.Action.InvoiceCorrection
import fi.espoo.evaka.shared.security.Action.MessageAccount
import fi.espoo.evaka.shared.security.Action.MobileDevice
import fi.espoo.evaka.shared.security.Action.OtherAssistanceMeasure
import fi.espoo.evaka.shared.security.Action.Pairing
import fi.espoo.evaka.shared.security.Action.Parentship
import fi.espoo.evaka.shared.security.Action.Partnership
import fi.espoo.evaka.shared.security.Action.Payment
import fi.espoo.evaka.shared.security.Action.PedagogicalDocument
import fi.espoo.evaka.shared.security.Action.Person
import fi.espoo.evaka.shared.security.Action.Placement
import fi.espoo.evaka.shared.security.Action.PreschoolAssistance
import fi.espoo.evaka.shared.security.Action.PreschoolTerm
import fi.espoo.evaka.shared.security.Action.ScopedAction
import fi.espoo.evaka.shared.security.Action.ServiceApplication
import fi.espoo.evaka.shared.security.Action.ServiceNeed
import fi.espoo.evaka.shared.security.Action.UnscopedAction
import fi.espoo.evaka.shared.security.Action.VoucherValueDecision
...
@JsonPropertyOrder(
    value = [
        ...
        "admin", "reportViewer", "director", "financeAdmin", "financeStaff", "serviceWorker", "messaging",
        "unitSupervisor", "staff", "specialEducationTeacher", "earlyChildhoodEducationSecretary",
    ],
)
...
private val actionRuleMappings = TrevakaActionRuleMapping().let {
    mapOf(
...
class TrevakaActionRuleMappingTest {
     ...
     *
     * Steps to transform src/test/resources/permissions/tampere-region.csv into tampere-region.xlsx with LibreOffice:
     * 1. View -> Freeze Cells -> Freeze First Row
     * 2. (Select first row) -> Format -> Text -> Bold
     * 3. (Select columns from D to N, i.e. all roles): Format -> Cells... -> Alignment -> Text Alignment -> Horizontal -> Center -> OK
     * 4. File -> Save As... -> (Select format: Office Open XML Spreadsheet (.xlsx)) -> Save
     */
    @Test
    fun csv() {
        val mapper = CsvMapper()
        val schema = mapper.schemaFor(CsvRow::class.java)
            .withHeader().withQuoteChar('"').withColumnSeparator(';')
        assertEquals(
            ClassPathResource("permissions/tampere-region.csv").getContentAsString(StandardCharsets.UTF_8),
            "\uFEFF${mapper.writer(schema).writeValueAsString(rows().map { it.toCsvRow() })}",
        )
    }
}

private fun rows() = Global.entries.map { Row("", translate(it), permissions(it)) }
    .sortedWith(compareBy({ actionType(it) }, { it.action.second })) +
    Application.entries.map { Row("Hakemus", translate(it), permissions(it)) } +
    ApplicationNote.entries.map { Row("Hakemuksen muistiinpano", translate(it), permissions(it)) } +
    ...
    AbsenceApplication.entries.map { Row("Esiopetuksen poissaolohakemus", translate(it), permissions(it)) } +
    ServiceApplication.entries.map { Row("Palveluntarpeen muutoshakemus", translate(it), permissions(it)) } +
    ...
    ChildStickyNote.entries.map { Row("Lapsen huomiot lähipäivinä", translate(it), permissions(it)) } +
    ...
    DailyServiceTime.entries.map { Row("Päivittäinen varhaiskasvatusaika", translate(it), permissions(it)) } +
    AssistanceFactor.entries.map { Row("Tuen kerroin", translate(it), permissions(it)) } +
    AssistanceNeedVoucherCoefficient.entries.map { Row("Palvelusetelikerroin", translate(it), permissions(it)) } +
    DaycareAssistance.entries.map { Row("Tuen taso varhaiskasvatuksessa", translate(it), permissions(it)) } +
    ...
    FeeThresholds.entries.map { Row("Talouden maksuasetukset", translate(it), permissions(it)) } +
    ...
    InvoiceCorrection.entries.map { Row("Laskun korjausrivi", translate(it), permissions(it)) } +
    Payment.entries.map { Row("Maksu", translate(it), permissions(it)) }

private fun permissions(action: UnscopedAction) = permissions(
    actionRuleMappings.mapValues { (_, actionRuleMapping) ->
        actionRuleMapping.rulesOf(action).toList().flatMap { roles(it) }.toSet()
    },
)

private fun permissions(action: ScopedAction<*>) = permissions(
    actionRuleMappings.mapValues { (_, actionRuleMapping) ->
        actionRuleMapping.rulesOf(action).toList().flatMap { roles(it) }.toSet()
    },
)

private fun permissions(rolesByMunicipality: Map<String, Set<UserRole>>) = UserRole.entries.associateWith { role ->
    val enabledMunicipalities = rolesByMunicipality.entries
...
private fun roles(rule: ScopedActionRule<*>) = when (rule) {
    is HasGlobalRole -> rule.oneOf
    is DatabaseActionRule.Unscoped<*> -> roles(rule)
...
private fun translate(action: Global) = action.name to when (action) {
    ...
    Global.FETCH_INCOME_STATEMENTS_AWAITING_HANDLER -> "Tuloselvitysten luku (odottaa käsittelyä)"
    ...
    Global.WRITE_SERVICE_WORKER_APPLICATION_NOTES -> "Palveluohjauksen muistiinpanojen muokkaus"
    ...
    Global.READ_SELECTABLE_FINANCE_DECISION_HANDLERS -> "Talouden päätöksen käsittelijöiden luku"
    Global.READ_PERSONAL_MOBILE_DEVICES -> "Henkilökohtaisten mobiililaitteiden luku"
    Global.CREATE_PERSONAL_MOBILE_DEVICE_PAIRING -> "Henkilökohtaisen mobiililaitteen parituksen luonti"
    ...
    Global.READ_ENDED_PLACEMENTS_REPORT -> "Varhaiskasvatuksessa lopettavat lapset -raportti"
...
private fun translate(entry: Enum<*>) = entry.name to when (entry.name) {
    ...
    "GENERATE_RETROACTIVE_VOUCHER_VALUE_DECISIONS" -> "Takautuvan arvopäätösluonnoksen luonti"
    "MERGE" -> "Yhdistäminen"
    ...
    "UPSERT_STAFF_OCCUPANCY_COEFFICIENTS" -> "Henkilöstön täyttö/käyttöasteen kertoimien muokkaus"
    ...
    "READ_ASSISTANCE_NEEDS_AND_ACTIONS_REPORT" -> "Lasten tuen tarpeet ja tukitoimet -raportti"
    "READ_ASSISTANCE_NEEDS_AND_ACTIONS_REPORT_BY_CHILD" -> "Lasten tuen tarpeet ja tukitoimet -raportti (lapsittain)"
    "READ_ATTENDANCE_RESERVATION_REPORT" -> "Päiväkohtaiset lapsen tulo- ja lähtöajat -raportti"
...
private data class Row(val typeName: String, val action: Pair<String, String>, val permissions: Map<UserRole, String>) {
    fun toCsvRow() = CsvRow(
        typeName = typeName,
        ...
        earlyChildhoodEducationSecretary = permissions.getValue(UserRole.EARLY_CHILDHOOD_EDUCATION_SECRETARY),
    )
}

private fun actionType(row: Row) = when {
