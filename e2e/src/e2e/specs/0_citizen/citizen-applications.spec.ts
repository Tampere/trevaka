...
test.describe('Citizen applications page', () => {
  ...
  test('Applications and ApplicationCreation customizations', async ({ evaka: page }) => {
    ...
    await expect.poll(() => page.find('h1 + p').text).toEqual('Lapsen huoltaja voi tehdä lapselleen hakemuksen varhaiskasvatukseen, esiopetukseen ja kerhoon. Huoltajan lasten tiedot haetaan tähän näkymään automaattisesti Väestötietojärjestelmästä.')
    ...
    await expect(applicationRadios).toHaveCount(3)
    await expect.poll(
    ...
    await expect.poll(() => page.find('[data-qa="daycare-expanding-info-text"]').text).toEqual('Varhaiskasvatushakemuksella haetaan paikkaa kunnallisesta päiväkodista tai perhepäivähoidosta, ostopalvelupäiväkodista tai palvelusetelillä tuetusta päiväkodista.')
  ...
  test('Daycare application form customizations', async ({ evaka: page }) => {
    ...
    await expect(page.find('[data-qa="urgent-file-upload"]')).toBeVisible()
    ...
    await expect(fullTimeRadio).toBeVisible()
    await expect(partTimeRadio).toBeVisible()
    ...
    await expect(page.find('[data-qa="startTime-input"]')).toBeHidden()
    await expect(page.find('[data-qa="endTime-input"]')).toBeHidden()
    // Check texts
