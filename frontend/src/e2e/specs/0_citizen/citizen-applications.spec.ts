...
describe('Citizen applications page', () => {
  ...
  test('Daycare application form customizations', async () => {
    ...
    await waitUntilEqual(
      () => page.find('[data-qa="startdate-instructions-text"]').innerText,
    ...
    await waitUntilEqual(
      () => page.find('[data-qa="urgent-input"] + div + p').innerText,
    ...
    await waitUntilEqual(
      () => fullTimeOptions.first().find('label').innerText,
      'Kokopäiväinen'
    )
    await waitUntilEqual(
      () => fullTimeOptions.nth(1).find('label').innerText,
      'Kokopäiväinen, 10 pv sopimus'
    )
    await waitUntilEqual(
      () => fullTimeOptions.nth(2).find('label').innerText,
    ...
    await waitUntilEqual(
        () => partTimeOptions.first().find('label').innerText,
        'Osapäiväinen, max 20h viikossa'
    )
    await waitUntilEqual(
      () => partTimeOptions.nth(1).find('label').innerText,
      'Osapäiväinen, max 5h päivässä'
    )
    await waitUntilEqual(
      () => partTimeOptions.nth(2).find('label').innerText,
      'Osapäiväinen, max 5h päivässä; 10 pv sopimus')

    await waitUntilEqual(
      () => partTimeOptions.nth(3).find('label').innerText,
    ...
    await waitUntilEqual(
      () => page.find('[data-qa="assistanceNeedInstructions-DAYCARE-text"]').innerText,
    ...
    await waitUntilEqual(
      () => page.find('[data-qa="duplicate-application-notification"]').innerText,
  ...
  test('Club application form customizations', async () => {
    ...
    await waitUntilEqual(
      () => page.find('[data-qa="wasOnDaycare-info-text"]').innerText,
    ...
    await waitUntilEqual(
      () => page.find('[data-qa="wasOnClubCare-info-text"]').innerText,
    ...
    await waitUntilEqual(
      () => page.find('[data-qa="assistanceNeedInstructions-CLUB-text"]').innerText,
